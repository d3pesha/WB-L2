...
func (ss *Server) HandlerEventsForDay(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Println(date)
		events, err := ss.store.ParseEvent(date, 1)
		if err != nil {
			http.Error(w, err.Error(), 503)
			return
		}
...
// обработка запроса ивента на неделю
	...
	func (ss *Server) HandlerEventsForWeek(w http.ResponseWriter, r *http.Request) {
		...
		events, err := ss.store.ParseEvent(date, 7)
		if err != nil {
			http.Error(w, err.Error(), 503)
			return
		}
...
// обработка запроса ивента на месяц
	...
	func (ss *Server) HandlerEventsForMonth(w http.ResponseWriter, r *http.Request) {
		...
		events, err := ss.store.ParseEvent(date, 30)
		if err != nil {
			http.Error(w, err.Error(), 503)
			return
		}
...
func NewServer(eventService service.EventService) *CalendarHandler {
	return &CalendarHandler{eventService: eventService}
}
...
// Вспомогательная функция сериализации объектов JSON
func writeResponse(w http.ResponseWriter, v interface{}) {
	resultJSON := struct {
		Result interface{}
	}{Result: v}
...
func (ch *CalendarHandler) HandlerCreateEvent(w http.ResponseWriter, r *http.Request) {
	_, date, mes, err := handlerDataPost(r)
	if err != nil {
		http.Error(w, err.Error(), 400)
		return
	...
	id := ch.eventService.CreateEvent(date, mes)
	writeResponse(w, id)
}

func (ch *CalendarHandler) HandlerUpdateEvent(w http.ResponseWriter, r *http.Request) {
	id, date, mes, err := handlerDataPost(r)
	if err != nil {
		http.Error(w, err.Error(), 400)
		return
	...
	event, err := ch.eventService.UpdateEvent(id, date, mes)
	if err != nil {
		http.Error(w, err.Error(), 503)
		return
	}
...
func (ch *CalendarHandler) HandlerDeleteEvent(w http.ResponseWriter, r *http.Request) {
	id, _, _, err := handlerDataPost(r)
	if err != nil {
		http.Error(w, err.Error(), 400)
		return
	...
	errDelete := ch.eventService.DeleteEvent(id)
	if errDelete != nil {
		http.Error(w, errDelete.Error(), 503)
		return
	}
...
func (ch *CalendarHandler) HandlerEventsForDay(w http.ResponseWriter, r *http.Request) {
	date := handlerDataGet(r)

	events, err := ch.eventService.ParseEvent(date, 1)
	if err != nil {
		http.Error(w, err.Error(), 503)
		return
	}
...
func (ch *CalendarHandler) HandlerEventsForWeek(w http.ResponseWriter, r *http.Request) {
	date := handlerDataGet(r)

	events, err := ch.eventService.ParseEvent(date, 7)
	if err != nil {
		http.Error(w, err.Error(), 503)
		return
	}
...
func (ch *CalendarHandler) HandlerEventsForMonth(w http.ResponseWriter, r *http.Request) {
	date := handlerDataGet(r)

	events, err := ch.eventService.ParseEvent(date, 30)
	if err != nil {
		http.Error(w, err.Error(), 503)
		return
	}
